...
Let's take CO2 Equvivalent as the target variable, resource as input variables as example for visulization (click [Here](https://weiquanluo.github.io/img/plotly_GHG_TotalCO2.html)).
...
For instance, we are interested in what is the outlier of the industries to produce CO2 Equvivalent. A DBSCANS algorithm is to cluster the industries. The clearn data for CO2 Equvivalent is obtained from the `bestglm_list` object above.
...
We feed the data fitting linear regression model for CO2 Equvivalent to the cluster algorithm in python
